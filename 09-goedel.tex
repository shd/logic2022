...
\begin{thm}Формальная арифметика с классической моделью --- неполна\end{thm} \pause
\begin{proof}
 Полная теория --- теория, в которой любая общезначимая формула доказуема. \pause
...
Формулировка 1 теоремы Гёделя о неполноте арифметики: <<если Ф.А. непротиворечива, то недоказуемо $\sigma(\overline{\ulcorner\sigma\urcorner})$. \pause
...
То есть, если $\vdash\text{Consis}$, то $\vdash\sigma(\overline{\ulcorner\sigma\urcorner})$, хотя
